...
If code were to be pushed onto a different branch than main, the workflow wouldn't run at all, as it is onl triggered when changes are made to main. Workflow doesn't trigger unless the changes are merged into main and a push to main triggers the workflow.